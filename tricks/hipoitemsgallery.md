...
- галерея из карточек товаров
- в каждой карточке есть картинка, название, описание (при наличии), цена
...
На данный момент (конец мая 2024) в базовом виджете все товары автоматически показываются строго отсортированными по названию: например, сначала начинающиеся на числа, потом с маленькой лат. буквы, заглавной лат. буквы, маленькой кириллицы, заглавной кириллицы. (0-9 a-z A-Z а-я А-Я). Мне показалось это не очень логично для товаров, поэтому мы это тоже исправим. (Хипо обещают, что потом сделают другую сортировку).
...
Дальше под этим виджетом добавляем блок HTML, в редакторе текста удаляем всё, что там вставляется и заменяем на этот код:
```html
...
В превью должно быть вот так - закрытый контент, потом блок HTML, в котором видно заголовок магазина:
